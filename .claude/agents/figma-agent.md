...
tools: Read, Grep, Glob, WebFetch, WebSearch, mcp__claude_ai_Figma, mcp__figma-dev-mode-mcp-server
model: sonnet
permissionMode: plan
maxTurns: 30
effort: high
...
- Figma MCP 서버는 Claude Code에 `claude.ai Figma`(`mcp__claude_ai_Figma__*`) 또는 `figma-dev-mode-mcp-server`(`mcp__figma-dev-mode-mcp-server__*`) 이름으로 등록되어 있어야 한다. 도구 이름이 이와 다르면 서버 이름 불일치로 간주하고 즉시 blocker로 보고한다(추측 재시도 금지).
